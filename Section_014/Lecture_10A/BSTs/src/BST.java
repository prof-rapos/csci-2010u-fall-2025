...
public class BST {
  ...
  private boolean dfsSearchRecursive(Node node, int target) {
    //if leaf reached, it is not found in this branch (our base case)
    if (node == null) {
      return false;
    }
  ...
  private Node deleteRecursive(Node node, int data) {
    ...
    if (data < node.data) {
      ...
      node.right = deleteRecursive(node.right, inOrderSuccessor.data);
    }
  ...
  public static void main(String[] args) {
    ...
    // DFS Search
    System.out.println("DFS Search for 11: " + bst.dfsSearch(11)); // true
